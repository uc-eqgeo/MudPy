...
rupture_list = glob(f'{rupture_dir}\\hikkerk3D_locking_NZNSHMscaling*.rupt')[::500]
...
def create_g_matrix(n_patches, n_ruptures, ruptures):
    print(f"Creating rupture G matrix... ({0}/{n_ruptures})", end='\r')
    G = np.zeros((n_patches, n_ruptures))
    start = time()
    for ix, rupture_file in enumerate(ruptures):
        ...
        print(f"Creating rupture G matrix... ({ix + 1}/{n_ruptures}) {(time() - start)/(ix + 1):.5f}", end='\r')
    print('')
    return G

G = create_g_matrix(deficit.shape[0], len(rupture_list), rupture_list)

target = np.random.randint(1,10,len(rupture_list))
deficit = np.matmul(G, target)

class deficitInversion:
    def __init__(self, rupture_list: list, deficit: np.ndarray, G: np.ndarray, b: float, N: float):
        ...
        self.GR_weight = 0  # Weighting for GR-rate misfit
        self.slip = G  # I*J matrix of rupture slip, where I is the number of patches and J is the number of ruptures
        ...
        self.Mw, self.Mw_bins = self.find_magnitudes()  # Magnitude of each rupture and magnitude_bins
    ...
    def find_magnitudes(self):
        ...
        min_Mw = binned_Mw[0]
        max_Mw = binned_Mw[-1]
        Mw_diff = min(np.diff(binned_Mw))
        binned_Mw = np.arange(min_Mw, max_Mw + Mw_diff, Mw_diff)  # Requested magnitudes, to be used as bins for GR-rate (in case mag bin is missing)
        return np.array(Mw), binned_Mw
    ...
    def get_bounds(self):
        ...
        upper_bound = [20] * self.n_ruptures  # Upperbound based on b-value, but 100 5Mw events a year
...
inversion = deficitInversion(rupture_list, deficit, G, b, N)
...
initial_rates_n = np.ones_like(target) * 0.5
...
try:
    ...
    deficit[:, 9] = inversion.deficit
    outfile = rupture_dir + f'\\synth_deficit_n{inversion.n_ruptures}.inv'
    ...
    deficit[:, 8] = reconstructed_deficit / (inversion.deficit + 1e-3)  # Fractional
    deficit[:, 9] = reconstructed_deficit - inversion.deficit  # Absolute deficit
    ...
    print("You're just testing the algorithm, so no output files are being created.")