...
def plot_hillshade_niwa(ax, alpha: float = 0.3, vertical_exaggeration: float = 0.01, clip_bounds: list = None,
                        ...
                        kermadec: bool = False):
    bathydir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "data", "bathymetry"))
    hillshade = os.path.join(bathydir, "niwa_nztm.tif")
    if kermadec:
        hillshade = hillshade.replace(".tif", "_kermadec.tif")
...
def plot_background(figsize: tuple = (6.4, 4.8), hillshading_intensity: float = 0.0, bounds: tuple = None,
                    plot_rivers: bool = True, plot_lakes: bool = True, hillshade_fine: bool = False, hillshade_kermadec: bool = False,
                    ...
                    disp_slip_max: float = 1., cum_slip_max: list = [5., 10.], step_size: list = None, tide: dict = {'time': 0}, logScale: bool = True):
        ...
        for main_ax in main_ax_list:
            if aotearoa:
                if hillshading_intensity > 0:
                    ...
                    if hillshade_fine:
                        plot_hillshade_niwa(main_ax, hillshading_intensity, clip_bounds=plot_bounds, cmap=hillshade_cmap, kermadec=hillshade_kermadec)
                    else:
        ...
        if plot_crust_cbar:
            if plot_sub_cbar:
                ...
                crust_ax = ax["cbar"]
            crust_cbar = fig.colorbar(crust_mappable, cax=crust_ax, extend='max')
        ...
        if pickle_name is not None:
            ...
            fig.savefig(pickle_name.replace('.pkl', '.png'), format="png", dpi=100)
